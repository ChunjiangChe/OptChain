...
if [ "$PROTOCOL" == "optchain" ]; then
    CMD="$CMD --shardId $SHARD_ID --nodeId $NODE_ID --experNumber $EXPER_NUMBER --experIter $EXPER_ITER --shardNum $SHARD_NUM --shardSize $SHARD_SIZE --blockSize $BLOCK_SIZE --symbolSize $SYMBOL_SIZE --propSize $PROP_SIZE --avaiSize $AVAI_SIZE --eReq $EX_REQ_NUM --iReq $IN_REQ_NUM --k $K --tDiff $TX_DIFF --pDiff $PROP_DIFF --aDiff $AVAI_DIFF --iDiff $IN_AVAI_DIFF --oDiff $ORDER_DIFF"
